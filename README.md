...
Одной из основных преимуществ Redux является разделение состояниея (данных) с презентационными и логическими уровнями. Из-за разделения обязанностей дизайн схемы состояния может выполняться отдельно от дизайна пользовательского интерфейса и любых сложных логических потоков.
...
В этой реализации каждый «родительский» reducer должны знать о любых action, используемых в дочерних. Любые изменения или дополнения потребуют от нас проверки нескольких reducer на изменение кода. Это ломает все преимущества от разделения reducer и значительно усложняет наш код.
...
const recipesReducer = (recipes, action) => {
  ...
  // Обратимся к reducer, который обрабатывает ингридиенты
...
В этой реализации мы разделяем логику редуктора на две части, одина из которых позволяет запускать любые дочерние reducer, а вторая - обрабатывать action самого reducer. 
...
Кроме того, поскольку этот код (или аналогичный) будет использоваться для пользовательского интерфейса, любые изменения в структуре состояния необходимо будет отражать не только в reducer, но и в самом пользовательском интерфейсе. Такой подход нарушает наше разделение о том, что касается модели и может потребовать значительных изменений в уровне пользовательского интерфейса, если изменится структура состояния.
