...
В качестве упражнения предлогаем вам самим добавить поддержку для action собственных сообщений об ошибке (способ для action creator указать, что ему нужено отправить другую ошибку, а не объект ошибки по умолчанию).